namespace StudentCareSystem.Application.Services.Job.ManualJob
{
    ...
    public class FailedSubjectManuallyEmailSendJobStrategy : IJobExecutionStrategy
    {
        ...
        public async Task ExecuteJobForTenantAsync(IServiceProvider serviceProvider, object? parameter = null)
        {
            ...
            var request = parameter as SendFailedSubjectEmailDto
                ?? throw new ArgumentNullException(nameof(parameter), "SendFailedSubjectEmailDto is required");
            await emailService.SendFailedSubjectNotificationsAsync(request);
        }
    }
}
